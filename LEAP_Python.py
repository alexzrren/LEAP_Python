...
class LEAP:
    def __init__(self, pseudotime_dir, exprmatx_dir):
        self.pseudotime_dir = pseudotime_dir
        self.exprmatx_dir = exprmatx_dir

    def MAC_lags(self, max_lag_prop=1/3, symmetric=True):
        ###############################################
        # 数据读入
        ###############################################
        pseudo_Time = pd.read_csv(self.pseudotime_dir, header=0, names=[
                                  'cell', 'PseudoTime1', 'PseudoTime2'])
        pseudo_Time = pseudo_Time.fillna(0)
        raw_Matx = pd.read_csv('ExpressionData.csv', index_col=0, header=0)

        pseudo_Time['PseudoTime'] = pseudo_Time['PseudoTime1'] + \
            pseudo_Time['PseudoTime2']

        # 为什么要排序？没看太懂
        pseudo_Time.sort_values(
            by=['PseudoTime', 'cell'], ascending=True, inplace=True)
        ...
        data_0 = expr_Matx.iloc[:, 0:int(window)]
        ...
        for i in range(int(max_lag)):
            ...
            cor_lag = (cent_0.T.dot(cent_lag) - 1/window * rowsumx_0.T.dot(rowsumx_lag)) / (np.sqrt(rowsumx2_0 -
                                                                                                    np.power(rowsumx_0, 2) / window).T.dot(np.sqrt(rowsumx2_lag - np.power(rowsumx_lag, 2) / window)))

            # Keep cor only if greater than previous lag cor
            max_lag_cor[max_lag_cor.abs() < cor_lag.abs(
            )] = cor_lag[max_lag_cor.abs() < cor_lag.abs()]
        ...
        if symmetric:
            ...
            MACs_greatest.iloc[coor_x,
                               coor_y] = max_lag_cor.iloc[coor_x, coor_y]
            lag_greatest.iloc[coor_x, coor_y] = lag_max.iloc[coor_x, coor_y]
            MACs_greatest.iloc[-coor_x,
                               coor_y] = max_lag_cor_t.iloc[-coor_x, coor_y]
            lag_greatest.iloc[-coor_x,
                              coor_y] = lag_max_t.iloc[-coor_x, coor_y]
        np.fill_diagonal(MACs_greatest, np.nan)
        np.fill_diagonal(lag_greatest, np.nan)
        result = pd.DataFrame(
            MACs_greatest, index=gene_names, columns=gene_names)
        return result
